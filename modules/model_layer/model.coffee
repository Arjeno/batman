...
Query = require './query'
{
  ...
  developer,
  mixin, typeOf, extend, functionName, getPath
} = require 'foundation'
...
class Model extends BatmanObject
  ...
  @persist: (mechanism, options...) ->
    ...
    mixin(mechanism, options...) if options.length > 0
  ...
  @encode: (keys..., encoderOrLastKey) ->
    @::initializeBatman()
    @::_batman.encoders ||= new SimpleHash
    encoder = {}
    ...
    for key in keys
      encoderForKey = extend({as: key}, @defaultEncoder, encoder)
  ...
  @classAccessor 'resourceName',
    get: ->
      if @resourceName?
        ...
        developer.error("Please define #{functionName(@)}.resourceName in order for your model to be minification safe. (For example, `@resourceName: '#{helpers.underscore(functionName(@))}'`)") if Batman.config.minificationErrors
        helpers.underscore(functionName(@))
  ...
  @_doStorageOperation: (operation, options, callback) ->
    developer.assert @::hasStorage(), "Can't #{operation} model #{functionName(@)} without any storage adapters!"
    adapter = @::_batman.get('storage')
    adapter.perform(operation, this, options, callback)

  for operationName in ['find', 'load', 'create']
    @[operationName] = Property.wrapTrackingPrevention(@[operationName])
  ...
  class Model.InstanceLifecycleStateMachine extends DelegatingStateMachine
    @transitions
      load:
        from: ['dirty', 'clean']
        to: 'loading'
  ...
  constructor: (idOrAttributes = {}) ->
    developer.assert  @ instanceof BatmanObject, "constructors must be called with new"
  ...
  @accessor 'lifecycle', -> @lifecycle ||= new Model.InstanceLifecycleStateMachine('clean', @)
  @accessor 'attributes', -> @attributes ||= new Hash
  @accessor 'dirtyKeys', -> @dirtyKeys ||= new Hash
  @accessor '_dirtiedKeys', -> @_dirtiedKeys ||= new SimpleSet
  @accessor 'errors', -> @errors ||= new ErrorsSet
  ...
  loadWithOptions: (options, callback) ->
    ...
    if @get('lifecycle').load()
      ...
      if @get('lifecycle.state') is 'loading' && !hasOptions
        ...
        err = new StateMachine.InvalidTransitionError("Can't load while in state #{@get('lifecycle.state')}")
        callback?(err)
        return Promise.reject(err)
  ...
  save: (options, callback) ->
    ...
    if !@get('lifecycle').startTransition(startState)
      error = new StateMachine.InvalidTransitionError("Can't save while in state #{@get('lifecycle.state')}")
      callback?(error)
    ...
    new Promise (fulfill, reject) =>
      @validate (error, errors) =>
        ...
        payload = extend({}, options, {data: options})
  ...
  destroy: (options, callback) ->
    ...
    if !@get('lifecycle').destroy()
      error = new StateMachine.InvalidTransitionError("Can't destroy while in state #{@get('lifecycle.state')}")
      callback?(error)
      return Promise.reject(error)

    new Promise (fulfill, reject) =>
      payload = mixin({}, options, {data: options})
  ...
  for operationName in ['load', 'save', 'validate', 'destroy']
   @::[operationName] = Property.wrapTrackingPrevention(@::[operationName])
  ...
  _transaction: (visited, stack) ->
    ...
    for own key, value of attributes
      if value instanceof Model && !value.isTransaction
        ...
        developer.warn "You're passing a mutable object (#{key}, #{functionName(value.constructor)}) in a #{@constructor.name} transaction:", value
...
for associationName in AssociationCurator.availableAssociations
  do (associationName) =>
    Model[associationName] = (label, options) ->
      @initializeBatman()
      collection = @_batman.associations ||= new AssociationCurator(this)
      collection.add new Batman["#{helpers.titleize(associationName)}Association"](this, label, options)

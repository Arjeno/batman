...
{extend, typeOf} = require 'foundation'
{helpers} = require 'utilities'

module.exports = class RestStorage extends StorageAdapter
  ...
  @ModelMixin: extend({}, @BaseMixin,
    urlNestsUnder: (keys...) ->
      ...
      @url = (options) ->
        childSegment = @storageKey || helpers.pluralize(@get('resourceName').toLowerCase())
      ...
      @::url = ->
        url = ""
        childSegment = @constructor.storageKey || helpers.pluralize(@constructor.get('resourceName').toLowerCase())
        url = ""
        for key, plural of parents
          if typeOf(plural) is 'Object'
            for deepKey, deepPlural of plural
  ...
  @::after 'readAll', @skipIfError (env, next) ->
    ...
    unless typeOf(env.recordsAttributes) is 'Array'
  ...
  @::after 'get', 'put', 'post', 'delete', @skipIfError (env, next) ->
    if env.json?
      ...
      env.result = @extractFromNamespace(env.json, namespace)
    next()
